...
def UploadAction(event=None):
    ...
    if action:  # File upload
        ...
        test_sql_input = test_df.rename(columns={'Title/Subtitle':'title', "Author":"author",
                                                    ...
                                                    'ISBN':'isbn', "Publisher":'publisher', 'Page Count':'pageCount'})
        test_sql_input['seriesName'] = 'None'
        test_sql_input['seriesNumber'] = 'None'
        test_sql_input = test_sql_input[['isbn','title',"author",'genre',"subgenre","lexileLevel",
                                            'publisher', "copyrightDate","summary","type","form",
                                            'pageCount','seriesName','seriesNumber']]
        test_sql_input = test_sql_input.replace(np.nan, 'None')
        if isinstance(test_sql_input, pd.DataFrame):
            print('Yayo')
        else: print('zoinks')
        # cursor =  db_config.cursor()
        # SQL INSERT statement
        insert_stmt = """
            INSERT INTO lottaBooks (isbn, title, author, genre, subgenre, lexileLevel, publisher, copyrightDate, summary, type, form, pageCount, seriesName, seriesNumber)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """
                
        # SQL SELECT statement
        select_stmt = """
            SELECT COUNT(*) FROM lottaBooks WHERE isbn = %s
        """
                
        # Loop through each row in the dataframe
        for _, row in test_sql_input.iterrows():
            ...
            #     f"{row['lexileLevel']}, {row['publisher']}, {row['copyrightDate']}, {row['summary']}, {row['type']}, "
            ...
            values = (
                ...
                row['lexileLevel'],
                row['publisher'],
                row['copyrightDate'],  # Use formatted date string (or None if missing)
                row['summary'],
                row['type'],
                row['form'],
                row['pageCount'],
            ...
            if result[0] == 0:  # result[0] contains the count from the SELECT query
                ...
                #     f"{row['subgenre']}, {row['lexileLevel']}, {row['publisher']}, {row['copyrightDate']}, {row['summary']}, "
